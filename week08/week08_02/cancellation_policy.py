...
def get_current_condition(pairs, end):
    for pair in pairs:
        lower_condition, higher_condition = pair

        lower_date = end - timedelta(hours=lower_condition['hours'])
        upper_date = end - timedelta(hours=higher_condition['hours'])
...
def get_cancellation_policy(
    ...
    assert my_now.now() < start, 'Invalid booking start.'
    ...
    current = get_current_condition(paired_conditions, start)
